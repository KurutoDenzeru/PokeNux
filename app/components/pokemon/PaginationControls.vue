      ...
      <Pagination :total="total" :items-per-page="itemsPerPage" v-model:page="internalPage">
        <template #default>
          <PaginationContent>
            ...
            <PaginationItem v-if="startPage > 1" :value="1" :isActive="internalPage === 1" @click="go(1)">1</PaginationItem>

            <template v-for="p in pagesArray" :key="p">
              <PaginationItem :value="p" :isActive="p === internalPage" @click="go(p)">{{ p }}</PaginationItem>
            </template>

            <PaginationItem v-if="endPage < totalPages" :value="totalPages" :isActive="internalPage === totalPages" @click="go(totalPages)">{{ totalPages }}</PaginationItem>

            <PaginationNext :disabled="internalPage === pagesArray[pagesArray.length - 1]" @click="go(internalPage + 1)" />
...
.w-full.flex.justify-end { padding-right: 0.25rem }
</style>
